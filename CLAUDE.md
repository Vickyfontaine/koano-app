...
## Version 5.3 | Confidential & Proprietary | 2026
...
This inventory is kept current. As of this writing the backend spine, all four dashboards, the archive/calibration layer, the "every verdict live" work (Phase 1), scheduled monitoring (Phase 2), the vaporware-resolution pass (Phase 3), the dashboard-differentiator redesign (Phase 4), and the national-expansion/extensibility work (Phase 5) are all built and verified.
...
- **National expansion & extensibility (Phase 5):** removed KOANO's expansion ceiling and made the coverage story legible for a data-partner conversation. Six slices, each holding the hard constraint that a NYC verdict stays bit-identical:
  - **Slice 1 — out-of-market fabrications killed.** The six NYC parcel-keyed providers returned NYC-flavored `representative` stand-ins on a null/out-of-market BBL (e.g. a Chicago address got a "typical Brooklyn 350-permit profile"). Now they return coverage-absent (`data:null` + an honest layer-named note) — never a stand-in for the wrong market. `lib/providers/real/coverage.ts`.
  - **Slice 2 — verdict-reproducibility fixture (§07C).** Record/replay at the registry + LLM seam (`lib/testing/cassette.ts`); freezes provider results + raw completions, asserts the decision surface byte-identical on offline replay, wired into the pre-push gate. Distinguishes a code regression from model drift; proven to red on a deliberate regression.
  - **Slice 3 — the federal trio** (each re-recorded through the fixture after a reviewed diff): **FEMA National Risk Index** → risk-volatility (national hazard composite; `fema-nri.ts`), **HUD QCT/DDA** → regulatory-policy (LIHTC eligibility; `hud-qct-dda.ts`), **Census Building Permits Survey** → market-timing as market-wide supply CONTEXT (not a second read on the block; durable read-through cache `bps_cache`/migration-022; `census-bps.ts`). Introduced the verdict-inputs era marker `VERDICT_INPUTS_ERA='v2-federal-risk-supply'` (persisted via migration-021).
  - **Slice 4 — five-state provenance taxonomy (§06):** `live | partner | representative | fetch_failed | coverage_absent` (`lib/providers/provenance.ts`); dropped the never-implemented `modeled`; partner attribution + a mixed-run rollup that never collapses to the weakest label; the whole double-duty `=== 'representative'` UI class generalized to `isTrustedProvenance`.
  - **Slice 5 — the coverage map** (`CoverageMap.tsx`/`coverage.ts`): per-verdict, per-LAYER, states rendered DISTINCTLY (a market gap ≠ a transient failure ≠ a stand-in), each non-live layer naming the feed that fills it. The artifact for a data-partner conversation.
  - **National-provider audit (§14/§16) — the hardcoded-NYC class + omission-over-fabrication.** The coverage map surfaced it: a NATIONAL provider with a hardcoded region is invisible because NY is correct for every NYC test. Crime hardcoded `state/NY` and FHFA HPI hardcoded the NY MSA while ignoring the address — every non-NYC verdict silently got New York's data (HPI even tagged `live`). Both now derive geography from the resolved address (`us-states.ts`). And the ~10 national providers that fabricated a region-flavored stand-in on failure were converted to honest omission (`data:null`). FBI crime endpoint drift fixed + a real key wired (§14/§18). Migrations 021–022.
...
- `partner` — a third-party PARTNER feed. Carries the PARTNER's trust profile, not ours — attributed to them BY NAME (the `source` field), never presented as "live from a source we verified". Trusted, present data. (The ready seam for a partner adapter: a provider sets `provenance:'partner'` + `source:'<Partner Name>'`; no provider emits it yet. WHEN THE FIRST ONE DOES: several `isLive` render-gates check `=== 'live'` specifically and will silently DROP partner data — switch them to `isTrustedProvenance` then. The archive's live-only capture correctly stays `=== 'live'`. Full list in the memory note `partner-provenance-islive-gates`.)
...
**Surgical re-record for a narrow blast radius (prefer this over a full re-record whenever it applies).** When a change provably affects ONLY a downstream call and cannot touch any replayed one, do NOT full-re-record. A full record resamples the five specialists, and their temp-0 wobble (confidence ±1, several bands) buries the one intended change and can make a pure punctuation edit read as a confidence regression. Use the surgical mode instead: `beginReplayThenRecord` / `dumpUsedRecords` in `lib/testing/cassette.ts` (the `'replay-or-record'` mode), driven by `scripts/rerecord-surgical.ts`. It replays every frozen upstream call byte-for-byte and records ONLY the call whose input changed (dropping the superseded record), so the fixture diff is exactly the intended change with zero wobble. Worked example (the engine em-dash pass): rewording the structural-driver strings in `direction.ts` / `entitlement.ts`, which feed ONLY the synthesis narrator and never the specialists, re-recorded a single LLM call and left the verdict, confidence, and every agent band byte-identical, so the decision-surface diff was the one driver string and nothing else. Precondition, non-negotiable: the change must provably not affect any replayed call (as a text edit fed only to the narrator cannot move the specialists); if it could, use the full `record-fixture.ts`.
...
**Liquid glass = chrome ONLY, over visually rich backgrounds (Apple HIG spec).** Glass is a lens: it needs busy content BEHIND it to refract. Over a flat colour, or used as a content card, it has nothing to bend and collapses into exactly the grey/white haze it's meant to avoid. So glass belongs only on the CHROME layer (nav bars, toolbars, sidebars, sheets, popovers, alerts) over rich content — NEVER on content cards (feature cards, cluster cards, verdict/ledger panels, the hero dashboard, pricing tiers → all solid). Apple's "regular" band: `backdrop-filter: blur(20–40px) saturate(1.2–1.5)`, white/black fill 60–80%, a 1px hairline border, no heavy shadow. The ONLY live glass on the marketing site is the **Nav** (`Nav.tsx`), tuned to that band and applied only when scrolled — `blur(24px) saturate(1.4)`, white `rgba(255,255,255,0.72)`, hairline `--border-light` bottom border — so over the image heroes it refracts and over flat sections it stays a quiet white haze. `.glass-card-light` in globals.css is defined but deliberately UNUSED: there is no legitimate content-layer home for it.
...
### Marketing hero renders & layout tokens (render pass — do NOT let these drift)

The marketing heroes and CTA sections run on delivered image renders positioned by a shared token file and per-page slot assignments, all hand-tuned against the renders. If the token values or aspect ratios drift, the copy overflows into the buildings — treat them as locked.

- **Layout tokens — single source `src/components/marketing/heroLayout.ts`.** `HERO_CONTAINER_WIDTH 800` (the centered text column == the render's blank strip), `HERO_HEADLINE_WIDTH 760` (h1 cap), `HERO_SUBHEAD_WIDTH 640` (subhead/tagline cap): all composed against the /for/institutions render; every render-backed hero imports these, never inlines them. `HERO_PADDING_TOP 34` is the text-block top offset, tuned so the rendered **nav→number gap (~40px) equals the number→headline gap** on every hero (the ~6px delta is the number's line-box leading). `HERO_PADDING_SIDE 32`. `HERO_PADDING_BOTTOM 210` is LEGACY (heroes bottom-anchor their own images; /about had to override it to `0` — the 210 inflated the section past the viewport once the promise quote was added, dragging the render below the fold).
- **Render slots — WebP (upscaled ~4K, q90) in `public/renders/`.** `how-it-works.webp` → /intelligence hero; `pricing.webp` → /pricing hero; `for-developers` / `for-professionals` / `for-institutions` / `for-community.webp` → the four /for/* cluster heroes via ClusterLanding's `render` prop; `about.webp` → /about hero; `cta.webp` → every CTA-section background (`CtaBackground.tsx`, absolute `z-index:-1`, opaque white bg so it shows over pale-wash sections without changing the section's own colour, and never affects layout). **Aspect ratios are load-bearing** and must be preserved exactly on any re-upscale: Group-A skyline/park `1401:768`, Group-B clusters `1376:768`, about `1042:1008`, cta `~2.91:1`.
- **Hero fit mechanics (do not regress).** Group-A heroes (how-it-works, pricing) are a fixed-height section (`calc(100svh - var(--nav-h) + 6px)`) with the full-bleed image **bottom-anchored**, so the buildings/park always land on the fold and only the empty top sky crops on wide-short windows — `object-fit: cover` is deliberately NOT used (it would slice the content). Group-B cluster heroes cap to the below-nav viewport and top-anchor the copy. `--nav-h` (globals.css `:root`, `64px`) is the single nav-height source; the nav, the page spacers, and the hero calcs all read it — never hardcode 64px.
- **Homepage hero** stays dark / left-aligned, now two-column with `HeroDashboard.tsx` on the right: a looping animated panel replaying the REAL recorded 175 3rd St verdict (five agents → BUY, every figure sourced), driven by `exampleVerdict.ts`, on a **solid white card** (NOT glass — the dark hero background is flat).
- **/intelligence (how-it-works) order:** hero → **product-demo video slot** (16:9 placeholder, ready for a `<video>`) → architecture → agents → synthesis → verdict → CTA. The old neural-map "live agent network" section was removed. **Homepage:** the founding-promise ("$50M acquisition / Crown Heights") section was removed.

### Cluster display badges (C1–C4) are decoupled from cluster_id

The C-badge shown on the hero pages, pricing, and dashboard is a DISPLAY index in KOANO's positioning order — **C1 developers · C2 professionals · C3 institutions · C4 communities** — intentionally decoupled from the internal `cluster_id` (still `cluster_1/2/4/5`, unchanged for DB / pricing / onboarding). So the display "C3" is institutions, NOT a cluster_3 (there is still no cluster_3 id). In `clusters.ts` the `badge` field carries the display index; `.number` still mirrors the cluster_id and is DOM-id-coupled (`c4-history`, etc.) — do not repoint `.number` to the badge.
...
**Migrations: all applied through 022** (verified), and IRS migration is seeded (3,120 counties). Phase 4 added 017 (property location_confidence), 018 (never-captured archive integrity), 019 (`verdict_hidden` side table — hide/unhide is mutable curation, the `verdicts` table stays immutable append-only), 020 (`contamination_cache`). Phase 5 added 021 (`verdicts.method` + `inputs_era` — the verdict-engine markers for the calibration record; both nullable, persistVerdict fails gracefully if unapplied) and 022 (`bps_cache` — the shared read-through cache for the Census Building Permits Survey county file). `FBI_CRIME_API_KEY` is set in Vercel + `.env.local` (gitignored) — genuine national state-level crime; without it NYPD covers NYC and crime is omitted elsewhere (§14/§18). `NYC_OPEN_DATA_APP_TOKEN` and `NEXT_PUBLIC_MAPBOX_ACCESS_TOKEN` are set in Vercel; the GitHub-Action archive-liveness check needs `CRON_SECRET` as a repo secret. Remaining optional action: set free tokens (`NOAA_CDO_TOKEN`, `HUD_USER_TOKEN`) in `.env.local` AND Vercel for the additive climate/rent signals (env additions need a redeploy to take effect). The verdict engine is fully live for NYC without them.
...
- OMISSION OVER FABRICATION (Phase 5, applied across ALL national providers). On a live-call FAILURE or a missing input, a provider returns `data:null` — NEVER a region-flavored fabricated stand-in. Classify: a missing input it can't even attempt (no county/tract resolved, an unset optional token) omits as `live` (the omission rule); an attempted-and-failed call omits as `fetch_failed`. A plausible fabricated value is worse than a null, because a null is visibly missing while a fabrication (and these were NYC/Northeast-flavored) gets reasoned over. No `REPRESENTATIVE_FALLBACK` constants remain in the real providers.
- A NATIONAL provider must key its query on the RESOLVED ADDRESS (state_fips → USPS via `us-states.ts`, tract GEOID, county FIPS, lat/lon) — NEVER a hardcoded region. The hardcoded-NYC class is invisible because NY is correct for every NYC test: crime hardcoded `state/NY` and FHFA HPI hardcoded the NY MSA while ignoring the address, so every non-NYC verdict silently served New York's data (HPI even as `live`). When a national source has a better NYC-local read (NYPD 1-mile for crime), prefer the LOCAL source for NYC and use the national one as the FALLBACK — do NOT put the national source first, or a newly-added key silently downgrades NYC. The coverage map (§06 Slice 5) is what surfaces this class; audit any provider with a hardcoded constant, an ignored `_addr`, or an NYC-only fallback path.
...
*KOANO CLAUDE.md v5.3 | Confidential & Proprietary | 2026*
*v5.3 records the built reality through Phase 5 (national expansion & extensibility): the verdict-reproducibility fixture wired into the pre-push gate (§07C, asserts the NYC verdict bit-identical on every push), out-of-market coverage-absent instead of NYC-flavored fabrication (Slice 1), the federal trio FEMA NRI / HUD QCT-DDA / Census BPS (Slice 3, re-recorded through the fixture after reviewed diffs), the five-state provenance taxonomy live | partner | representative | fetch_failed | coverage_absent (§06, Slice 4), the per-layer coverage map for a data-partner conversation (Slice 5), and the national-provider audit — the hardcoded-NYC failure class (crime + FHFA HPI silently served New York's data everywhere; both now key on the resolved address), omission-over-fabrication across the national providers, and the FBI CDE endpoint-drift fix. Prior phases (v5.2) stand: backend spine, four dashboards, archive/calibration (07A), monitoring (07B), every verdict live (Phase 1), the dashboard-differentiator redesign (Phase 4). Migrations through 022.*
*The premium demo is built honestly today so that capital turns it into production tomorrow, without a rewrite and without a lie. A missing value is shown as missing; a national provider keys on the address, never a constant; and a fabricated number never gets reasoned over.*
